...
// gv def
void test_def_use_1(uint32_t *gv_ptr_1, uint32_t *gv_ptr_2, uint32_t gv_ptr_3){
  // gv_ptr_1 defined here
  // gv_ptr_2 used here
  *gv_ptr_1 = *gv_ptr_2 + gv_ptr_3;
  gv_4 = 5;
}
void lock(){
  BSP_LED_Off(LED4);
...
void handle_unlock_input(unsigned int one, struct test_struct test, unsigned int exp, unsigned int ms, unsigned int *unlock_count, unsigned int unused)
{
    ...
    while(1) {
        ...
        rx_from_uart(5);  //gdb调试此步骤时使用next而不是step
