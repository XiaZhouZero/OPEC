...
def encode_size(size):
    ...
    count -= 1  #Counts one to many and field is size -1
    # print("Size field value:%d"%count)
    return count
    ...
    if size > 128 and len(dis_sub) > 0:
        for sub in dis_sub:
            if sub >=0 and sub < 8:
                ...
                print("Sub Regions must be in range [0,7] is:%d"%sub)
                raise ValueError
    elif (size <= 128 and len(dis_sub) > 0):
        print("Subregions only allowed when size > 128/ size is:%d"%size)
        raise ValueError
    return attr
