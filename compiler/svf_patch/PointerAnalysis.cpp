...
static llvm::cl::opt<std::string> SVF_DDA_PTA_Results("dvf-pointer-analysis-results",
                                  llvm::cl::desc("JSON File to write dvf pointer analysis results to"),
                                  llvm::cl::init("build9/dvf_results.json"), llvm::cl::value_desc("filename"));
...
void PointerAnalysis::finalize()
{
    ...
    if(PTSAllPrint) {
        ...
        jsonFile.open(SVF_DDA_PTA_Results);
        jsonFile << PointerAnalysisJsonRoot;
        jsonFile.close();
    }
...
void PointerAnalysis::dumpPts(NodeID ptr, const PointsTo& pts)
{
    ...
    std::string ptr_name, location, function_name;
    bool is_in_func=false, not_func_arg = false;
    ...
    if (SVFUtil::isa<DummyObjPN> (node))
    {
		...
		if (node->hasValue()) {
            ptr_name = node->getValue()->getName();
            location = getSourceLoc(node->getValue());
			outs() << "##<" << node->getValue()->getName() << "> ";
			outs() << "Source Loc: " << getSourceLoc(node->getValue());
		}
	}
    ...
    if(const Function *func = node->getFunction()) {
        function_name = func->getName().str();
        outs() << function_name;
    }
    outs() << ")";
    ...
    std::size_t found = location.find("in line:");
    ...
    else
    {
                ...
                ++it)
        {
            ...
            outs() << *it << "(" << _node_kind << ") ";
            Json::Value target_node;
            // target_node["NodeType"] = _node_kind;
            // targets[*it] = target_node;
        }
        outs() << "}\n";
    }
        ...
        if (SVFUtil::isa<DummyValPN>(node))
			...
			if (!SVFModule::pagReadFromTXT()) {
				if (node->hasValue()) {
                    ...
                    Json::Value target_obj;
                    target_obj["loc"] = getSourceLoc(pagNode->getValue());
                    target_obj["name"] = pagNode->getValue()->getName().str();
    ...
    outs() << "\n";
    Json::Value ptr_node;
    ptr_node["ptr_name"] = ptr_name;
    ptr_node["location"] = location;
    ptr_node["targets"] = targets;
    if (is_in_func && not_func_arg) {
        if (!PointerAnalysisJsonRoot.isMember(function_name)) {
            Json::Value function_node;
            PointerAnalysisJsonRoot[function_name] = function_node;
        }
        PointerAnalysisJsonRoot[function_name].append(ptr_node);
    }
}
...
{
    outs() << "\nNodeID: " << getFunPtr(cs);
    outs() << "\nCaller:" << cs->getCallSite()->getFunction()->getName().str();
    outs() << "\nCallSite: ";
