 ...
 */
void AndersenStat::collectCycleInfo(PAG* pag, ConstraintGraph* consCG)
{
    _NumOfCycles = 0;
    _NumOfPWCCycles = 0;
        ...
        if (num > 1)
        {
            if(repNodes.insert(repNode).second)
            {
                ...
                if(consCG->isPWCNode(repNode)) {
                    _NumOfPWCCycles ++;
                    // ConstraintNode* pwcnode = consCG->getConstraintNode(repNode);
                    // PAGNode* pagnode = pag->getPAGNode(repNode);
                    // errs() << "PWCNode ID: " << repNode << ", name: " << pagnode->getValue()->getName().str() << "\n";
                }
            }
